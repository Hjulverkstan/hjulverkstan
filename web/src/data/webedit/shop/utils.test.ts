...
describe('isShopOpen', () => {
  ...
  it('handles overnight hours (close < open) correctly', () => {
    ...
    expect(isShopOpen(overnight, new Date(2024, 0, 1, 23, 0))).toBe(true);
    expect(isShopOpen(overnight, new Date(2024, 0, 1, 1, 0))).toBe(true);
    expect(isShopOpen(overnight, new Date(2024, 0, 1, 3, 0))).toBe(false);
  });
});
