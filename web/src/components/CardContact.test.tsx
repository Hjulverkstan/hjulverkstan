...
describe('CardContact Component', () => {
  ...
  test('should render correctly with full shop details and groups hours', () => {
    // Act
    const { container } = render(<CardContact shop={mockShop} />);
    ...
    expect(screen.getByText('monday–friday: 08:00 - 16:00')).toBeInTheDocument();
  ...
  test('should adjust layout for compact variant', () => {
    // Act
    const { container } = render(<CardContact shop={mockShop} variant="compact" />);
  ...
  test('should hide header when showHeader is false', () => {
    // Act
    render(<CardContact shop={mockShop} showHeader={false} variant="default" />);
