...
describe('MultiSelect Component', () => {
  ...
  test('should render hierarchy correctly', () => {
    ...
    expect(screen.getByTestId('item-child1')).toHaveAttribute('data-indented', 'true');
    expect(screen.getByTestId('item-root1')).toHaveAttribute('data-indented', 'false');
  });
  ...
  test('should set parent to intermediate if some children are selected', () => {
    ...
    // Assert
    expect(screen.getByTestId('item-parent1')).toHaveAttribute('data-checkbox', 'intermediate');
    expect(screen.getByTestId('item-child1')).toHaveAttribute('data-checkbox', 'checked');
    expect(screen.getByTestId('item-child2')).toHaveAttribute('data-checkbox', 'unchecked');
  });
  ...
  test('should set parent to checked if parent value is in selected', () => {
    ...
    // Assert
    expect(screen.getByTestId('item-parent1')).toHaveAttribute('data-checkbox', 'checked');
  ...
  test('should handle folding: selecting last child should select parent and remove children', async () => {
    ...
    expect(setSelected).toHaveBeenCalledWith(expect.arrayContaining(['parent1']));
    expect(setSelected).toHaveBeenCalledWith(expect.not.arrayContaining(['child1', 'child2']));
  });
