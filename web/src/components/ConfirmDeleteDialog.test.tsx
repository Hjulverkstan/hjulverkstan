...
describe('ConfirmDeleteDialog Component', () => {
  test('should render the entity name and title', () => {
    // Act
    render(
      <ConfirmDeleteDialog entity="Customer" entityId="C-42" />
    );

    // Assert
    expect(screen.getByText('Choose delete method')).toBeInTheDocument();
    expect(screen.getByText(/you are trying to delete customer with id c-42/i)).toBeInTheDocument();
  });
  ...
  test('should render entity without id when entityId is omitted', () => {
    ...
    // Assert
    expect(screen.getByText(/you are trying to delete ticket/i)).toBeInTheDocument();
  ...
  test('should disable Remove button and show tooltip when disable=true', () => {
    ...
    expect(screen.getByTestId('tooltip-content')).toHaveTextContent('Delete ticket first.');
  });
});
