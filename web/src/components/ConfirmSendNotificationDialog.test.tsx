...
describe('ConfirmSendNotificationDialog Component', () => {
  test('should render phone number and title', () => {
    // Act
    render(
      <ConfirmSendNotificationDialog phoneNumber="555-1234" onSend={vi.fn()} />,
    );

    // Assert
    expect(
      screen.getByText(/completing this repair will notify the customer/i),
    ).toBeInTheDocument();
  ...
  test('should call onSend when Confirm button is clicked', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: /complete & notify/i }),
    );
