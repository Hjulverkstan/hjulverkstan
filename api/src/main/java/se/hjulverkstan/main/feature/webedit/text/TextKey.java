...
public enum TextKey {
    ...
    private static final Map<String, TextKey> BY_KEY =
            Stream.of(values()).collect(Collectors.toMap(TextKey::getKey, e -> e));

    @JsonCreator
    public static TextKey fromKey(String key) {
        TextKey v = BY_KEY.get(key);
        if (v == null) throw new IllegalArgumentException("Unknown TextKey: " + key);
        return v;
    }
}
