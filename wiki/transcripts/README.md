...
Exemplars: [`2026-08-17-intro-pt1.md`](2026-08-17-intro-pt1.md) and [`-pt2.md`](2026-08-17-intro-pt2.md), the second written after the digest rule was named; [`2026-08-18-intro-pt3.md`](2026-08-18-intro-pt3.md) for a working session, and the first to run every step including the independent check of the clean; [`2026-08-19-client-meeting.md`](2026-08-19-client-meeting.md) for a conversational meeting with no folds of its own, where topics are gathered into topical folds that name their minutes.
...
3. **Body** — folds as nested headings, each suffixed ` · mm:ss`. Where the speaker folded, follow the speaker; where the meeting had no fold structure, impose topical folds, still timestamped — a topic that recurs through the hour may be gathered into one fold, headed by its first timestamp and naming the later minutes (` · 06:07, 37:19`), and the file may then run in topical rather than chronological order; the checker locates a claim anywhere in the file. *Lean (2026-08-19, the client meeting):* topical order is right where the recording has no folds of its own; a meeting with a speaker's own folds keeps them. `[?]` marks a guessed word or attribution; `[Ed: …]` an editorial note on a factual point, original wording kept.
...
- `2026-08-19-client-meeting.md` — the first meeting of this phase with the client: Chiaco, who leads Hjulverkstan at Save the Children, and Samir, with Jona, Mauricio and the interns. First source for the fold above `product/`. Second full run of the method: clean checked independently (two passes, clean on the second), written under an orchestrator's ruling in `.raw/…/ruling.md` (redaction on the record at 57:07–58:33; gathering of recurring topics; provisional pending Jona), writing checked three times, two rewrites. The recorder put every cue on Jona and the ball was not in use, so which Save the Children voice is whose is inferred `[?]`; the names came from Jona after the run (the recorder heard "Jacob" and "Samuel"). Status: pending approval — peer review in the PR by Chiaco, Samir, Mauricio and the interns.
